...
description: "Browse the risk bank as a hierarchical tree of groups, hazards, and controls. Expand nodes, search the tree, and open items to edit their details."
---
...
### Grid view: Open button and context menu

When the bank is shown as a data grid, the type cell on each row includes an **Open** button. Clicking **Open** launches the edit modal for that item — group, hazard, or control — using the action that matches its type.

You can also right-click any row to open a context menu with an **Edit** action. Both entry points open the same modal, so you can pick whichever is faster while scanning the grid.

Edit actions are disabled on rows that cannot be edited inline (for example, rows that need to be opened from their parent context).
